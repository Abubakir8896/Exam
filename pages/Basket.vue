...
<template>
    <div v-if="store.basket.length" class="container mt-[60px] mb-[80px]">
        <div class="relative w-[400px]">
            ...
            <div v-if="store.basket.length > 0" class="absolute top-6 right-[140px] text-[#fff] w-[22px] h-[18px] ">
                <h1 class="bg-[#C63C3C] text-center border rounded-full text-[12px]">{{ store.basket.length }}</h1>
            </div>
        </div>
                    ...
                    <div class="mt-[20px] pr-[70px]">
                        ...
                        <div class="mt-[10px] w-full flex justify-between items-center">
                            <div class="w-[280px] flex">
                                <img class="w-[80px] h-[80px] rounded-[15px]" :src="item.image" alt="">
                                <div class="ml-[40px]">
                                    <p class="text-primary text-[16px] line-clamp-1">{{ item.title }}</p>
                                    <p class="mt-[25px] font-semibold">{{ item.newPrice }}₽</p>
                                </div>
                            </div>
                            <p class="w-[250px] line-clamp-3">{{ item.description }}</p>
                            <p class="w-[250px] line-clamp-3">RAD-COMBO-50/XXX/230/XXX/XXX/S4/XS</p>
                            ...
                            <button @click="removeProductById(item.id)">
                                <img class="w-[20px] h-[20px]" src="../public/basketicon.png" alt="">
                            </button>
                        </div>
                    </div>
                </div>
        ...
        <div class="w-full h-[628px] p-[50px] bg-[#F2F2F2] mt-[30px] rounded-[20px]">
            <div>
                <h1 class="text-[32px]">Оформление</h1>
                <div class="flex justify-between mt-[20px] mb-[40px]">
                    <input class="p-[10px] w-[373px] bg-[#F2F2F2] border border-[#000] rounded-[100px] text-[#000]" required
                        placeholder="ФИО" v-model="order.full_name" type="text">
                    <input class="p-[10px] w-[373px] bg-[#F2F2F2] border border-[#000] rounded-[100px] text-[#000]" required
                        placeholder="телефон" v-model="order.phone" type="text">
                    <input class="p-[10px] w-[373px] bg-[#F2F2F2] border border-[#000] rounded-[100px] text-[#000]" required
                        placeholder="Электронная почта" email v-model="order.email" type="text">
                </div>
                <hr>
                <div class="mt-[30px]">
                    ...
                    <input
                        ...
                        required placeholder="Адрес доставки" v-model="order.address" type="text"><br>
                    <input
                        class="mt-[30px] p-[20px] pb-[100px] w-[767px] bg-[#F2F2F2] border border-[#000] rounded-[28px] text-[#000]"
                        required placeholder="Комментарий" v-model="order.comment" type="text">
                </div>
            </div>
        </div>
        <div class="w-full h-[361px] p-[50px] bg-[#F2F2F2] mt-[30px] rounded-[20px]">
            <h1 class="text-[32px]">Оплата</h1>
            <div class="flex w-[600px] justify-between mt-[30px]">
                <p class="text-[16px] text-primary/50">Товары.............................................{{
                    calculateTotalPrice() }}₽</p>
                <p class="text-[16px] text-primary/50">Доставка..............................................580₽</p>
            </div>
            <div class="mt-[35px]">
                <h1 class="text-[22px] fond-semibold">{{ calculateTotalPrice() + 580 }}₽</h1>
                <div class="mt-[30px] flex justify-between w-[650px] items-center">
                    <button @click="addorder()"
                        class="w-[324px] p-[8px] rounded-[100px] text-[#fff] bg-[#454545]">Купить</button>
                    <div class="flex items-center">
                        <input type="radio">
                        <p class="text-[12px] text-primary/50 ml-[5px]">Я согласен наобработку моих персональных данных</p>
                    </div>
                </div>
            </div>
        </div>

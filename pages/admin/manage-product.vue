...
async function deleteProduct(id) {
    axios.delete(`https://66861ed583c983911b00e13f.mockapi.io/product/${id}`)
        .then(response => {
            fetchProducts()
        })
        .catch(error => {
            ...
            <div v-for="item in data" :key="item.id" :data="item">
                <div class="relative w-[100%]  px-1 md:px-3 py-[30px] bg-white shadow-lg rounded-md">
                    ...
                    <div class="flex flex-col gap-5 pl-6 mt-[30px]">
                        <h2 class="text-[17px] max-w-[227px] text-[Manrope] line-clamp-1">{{ item?.title }}</h2>
                        <div class="flex justify-between items-center">
                            ...
                            <nuxt-link :to="`/admin/${item.id}`">
                                <button class="bg-[#fff] w-[55px] flex items-center p-[10px] rounded-[100px] border"> <img
                                        class="w-[15px] h-[16px] ml-[8px] mr-[8px]" src="../../public/adminpnaledit.png"
                                        alt=""></button>
                            </nuxt-link>
                            <button @click="deleteProduct(item.id)"
                                class="bg-[#454545] w-[55px] flex items-center p-[10px] rounded-[100px]">
                                <img class="w-[15px] h-[16px] md:h-[18px] ml-[11px] mr-[11px]"
